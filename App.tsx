...
const App: React.FC = () => {
  ...
  // Sync / load active creator
  useEffect(() => {
    const saved = localStorage.getItem('infinite_heroes_creator');
    if (saved) {
      try {
      ...
      fetch('/api/users', {
        ...
        body: JSON.stringify({ email: 'local-creator@infinite.multiverse' })
      })
      .then(r => r.json())
      .then(data => {
        if (data && data.id) {
          const userObj = { id: data.id, email: data.email };
          setActiveCreator(userObj);
          localStorage.setItem('infinite_heroes_creator', JSON.stringify(userObj));
        }
      })
      .catch(e => console.warn("Failed to auto-register default user, running in safe memory mode:", e));
    }
  }, []);
...
- ARC-RIVAL / VILLAIN: ${villainInstruction}
...
`;
  ...
  const generateImage = async (beat: Beat, type: ComicFace['type']): Promise<string> => {
    ...
    if (villainRef.current?.base64) {
        contents.push({ text: "REFERENCE 3 [ARC-RIVAL]:" });
    ...
    if (type === 'cover') {
        ...
        if (villainRef.current) {
            promptText += ` Looming threateningly in back, we see ARC-RIVAL [VILLAIN] (Use REFERENCE 3).`;
        }
        ...
        promptText += `INSTRUCTIONS: Maintain strict character likeness. If scene mentions 'HERO', you MUST use REFERENCE 1. If scene mentions 'CO-STAR' or 'SIDEKICK', you MUST use REFERENCE 2. If scene mentions 'ARC-RIVAL' or 'VILLAIN' or 'NEMESIS', you MUST use REFERENCE 3.`;
  ...
  const handleVillainUpload = async (file: File) => {
       try { 
           const base64 = await fileToBase64(file); 
           setVillain({ base64, desc: "The Arch Nemesis Villain" }); 
