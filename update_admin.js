const fs = require('fs');
const content = fs.readFileSync('AdminDashboard.tsx', 'utf8');
...
let newContent = content.replace('export default function AdminDashboard() {', featuresList + '\nexport default function AdminDashboard() {\n    const [showPlanModal, setShowPlanModal] = useState(false);\n    const [editingPlan, setEditingPlan] = useState<any>({ name: "", priceSubscription: 0, priceOneTime: 0, features: [] });');

const plansRegex = /\{activeTab === 'plans' && \([\s\S]*?(?=\{activeTab === 'integrations')/;

const newPlansTab = `{activeTab === 'plans' && (
                                    ...
                                    <div key={p.id} className="p-4 border border-slate-800 bg-slate-900 flex flex-col">
                                        ...
                                        <button className="text-red-500 hover:text-red-400 text-xs flex items-center justify-center gap-1 border border-red-900/50 py-1" onClick={async () => {
                                            if (confirm(\`Delete plan \${p.name}?\`)) {
                                                ...
                                                await fetch(\`/api/admin/plans/\${p.id}\`, { method: 'DELETE', headers: { 'Authorization': \`Bearer \${token}\` } });
                                                fetchData();
                                            }
...
newContent = newContent.replace(plansRegex, newPlansTab);
fs.writeFileSync('AdminDashboard.tsx', newContent);
