...
code = code.replace(
    "const [activeTab, setActiveTab] = useState<'dashboard' | 'users' | 'ai_config' | 'plans' | 'features'>('dashboard');",
    "const [activeTab, setActiveTab] = useState<'dashboard' | 'users' | 'ai_config' | 'plans' | 'features' | 'analytics'>('dashboard');\n    const [analyticsData, setAnalyticsData] = useState<any>(null);"
)

# 2. Fetch analytics data in loadAdminData
fetch_analytics = """
...
"""
code = code.replace("const systemData = await systemRes.json();\n            setHealthData(systemData);", "const systemData = await systemRes.json();\n            setHealthData(systemData);\n" + fetch_analytics)

# 3. Add Analytics Tab Button
old_tabs = r"<button\s*onClick=\{.*?setActiveTab\('features'\).*?\}\s*className=\{.*?activeTab === 'features'.*?\}>\s*UI Feature Flags\s*</button>"
new_tabs = """<button onClick={() => setActiveTab('features')} className={`px-4 py-2 font-bold transition-all ${activeTab === 'features' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-slate-400 hover:text-slate-200'}`}>
                                UI Feature Flags
                            </button>
                            <button onClick={() => setActiveTab('analytics')} className={`px-4 py-2 font-bold transition-all flex items-center gap-2 ${activeTab === 'analytics' ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-slate-400 hover:text-slate-200'}`}>
                                <span>$</span> Cost Analytics
                            </button>"""
code = re.sub(old_tabs, new_tabs, code, flags=re.DOTALL)

# 4. Add Analytics UI Tab Content
analytics_ui = """
                        {activeTab === 'analytics' && (
...
"""
# Inject it right before the plans tab
code = code.replace("{activeTab === 'plans' && (", analytics_ui + "\n                        {activeTab === 'plans' && (")

with open('AdminApp.tsx', 'w') as f:
    f.write(code)

print("AdminApp.tsx patched with Analytics UI")
