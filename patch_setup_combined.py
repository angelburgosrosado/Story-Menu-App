...
# Modify handleVaultGenerate & Add handleSaveToVault
...
# Find the end of the `if (activeTab === 'vault')` block
# which ends right before `             </div>\n        )}`
search_end = """                  )}
             </div>
        )}"""

end_idx = content.find(search_end, start_idx) + len("                  )}")
...
print("Setup.tsx permissions patched correctly")
