...
console.log(`📡 URL target: postgresql://angelburgosrosado:******@your_host:5432/comics-v1`);
...
async function run() {
    ...
    try {
        ...
        if (!tcpResult.ok) {
            ...
            console.error("\n💡 ANALYSIS: This indicates a network or firewall block. The host your_host is either offline, not accepting connection requests on port 5432, or its firewall/security lists have not whitelisted the Cloud Run container.");
            process.exit(1);
        }
